...
**Citation check.** カードが貼り付けたソースではなく私の知識から生成された場合、ルールと裏に引用された判例・制定法は AI モデルが生成したもので、検証されていません。カードを暗記する前に、アウトライン、ケースブック、またはリサーチツール(Westlaw、Fastcase、CourtListener)で確認してください。マスタリーにドリルされた誤ったカードは、カードがないより悪い。
